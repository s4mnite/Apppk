...
function BarcodeScanner({ onScan, onClose, darkMode }) {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      // En la app nativa (APK) hay que pedir el permiso de cámara de Android
      // explícitamente antes de intentar getUserMedia, o el WebView se queda
      // esperando sin mostrar ningún diálogo.
      if (Capacitor.isNativePlatform()) {
        try {
          const perm = await Camera.requestPermissions({ permissions: ["camera"] });
          if (perm.camera !== "granted" && perm.camera !== "limited") {
            setError("Necesitamos permiso de cámara para escanear. Actívalo en Ajustes del teléfono.");
            setLoading(false);
            return;
          }
        } catch (_) {
          // Si el plugin falla por algún motivo, igual intentamos con getUserMedia normal.
        }
      }
...
export default function App() {
  ...
  // Barra de estado nativa: dorado de marca, con íconos oscuros para que se lean bien
  useEffect(() => {
    if (!Capacitor.isNativePlatform()) return;
    StatusBar.setBackgroundColor({ color: "#ffc400" }).catch(() => {});
  ...
  // Sin esto, Android cierra la app entera al tocar atrás en vez de navegar
  // hacia la pantalla anterior, que es lo que cualquier usuario espera.
  useEffect(() => {
    if (!Capacitor.isNativePlatform()) return;
    const sub = CapacitorApp.addListener("backButton", () => {
      if (modal) { setModal(null); return; }
      if (boletaModal) { setBoletaModal(null); return; }
      if (showAdmin) { setShowAdmin(false); return; }
      if (activeNav !== "Dashboard") { setActiveNav("Dashboard"); return; }
      CapacitorApp.exitApp();
    });
    return () => { sub.then(s => s.remove()); };
  }, [modal, boletaModal, showAdmin, activeNav]);
